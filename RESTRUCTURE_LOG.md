...
## Batch 50 — Downgrade theoretical-contribution language to analytical observations (supervisor feedback)

**Date:** 2026-06-23
**Approved by:** user messages "proceed with Option A" and "commit and push this", following supervisor feedback on Ch 5 ("De teoretiske bidrag … virker primært som empiriske observationer, som med et bredere empirisk grundlag og en mere omfattende teoretisk bearbejdning potentielt kunne udvikles til teoretiske bidrag. Enten kunne de teoretiske bidrag foldes tydeligere ud og forankres stærkere teoretisk, eller også ville jeg præsentere dem som empiriske observationer.").
**Type:** Content edit (four files).

### Rationale

Supervisor's reflection on Ch 5: the four items in §`sec:disc-contrib` read as empirical observations drawn from a single Work Programme rather than as theoretical contributions in the strong sense. The supervisor offered two options — either deepen the theoretical anchoring across more material, or reframe the items as observations. Given the documentary scope (single corpus, no interviews, no comparison cases), option two is the more defensible move. This batch propagates the downgrade through every place the thesis uses the "four theoretical contributions" framing, so the framing is consistent across Abstract, Ch 1 Contributions, Ch 5 §7 discussion, and Ch 6 Contributions revisited. The "Empirical" and "Practical" contribution headings in Ch 1 and Ch 6 are left untouched — those characterise what the thesis genuinely does at empirical and practical levels.

### Changes by file

| File | Action | Note |
|---|---|---|
| `06_report/Frontmatter/Abstract.tex` | Edit (§6 Contribution paragraph) | "contributes to institutional theory by extending the literature on investment logics into the under-studied domain of supranational innovation policy" → "applies institutional theory to a setting the literature has largely passed over, supranational innovation policy, and develops four analytical observations from the corpus reading that warrant testing on broader empirical material". Second sentence (practical implications) preserved. |
| `06_report/Chapters/01_intro.tex` | Edit (§`sec:intro-contribution` Theoretical bullet) | "extends institutional-logics scholarship into supranational public-investment instruments" → "applies institutional-logics scholarship to supranational public-investment instruments"; "articulates two analytical refinements" → "develops two analytical distinctions that warrant testing on broader empirical material"; "candidate contributions to the wider literature on institutional devices" → "observations from the corpus rather than as settled contributions to the literature on institutional devices". Tail sentence added pointing to `\Cref{sec:conc-future}` for the cross-case work that would lift the observations to settled contributions. Empirical and Practical bullets unchanged. |
| `06_report/Chapters/05_discussion.tex` | Edit (overview paragraph + §7) | Overview line "articulates four theoretical contributions" → "sets out four analytical observations the corpus supports". §7 section renamed `\section{Theoretical contributions}` → `\section{Analytical observations}` (label `sec:disc-contrib` preserved so existing cross-references in Ch 6 still resolve). Lead sentence reframed to state the items are grounded in a single Work Programme. Four bullets softened in parallel: bullet 1 retitled "*Logic-as-register* as analytical distinction" with closing sentence flagging the empirical question a single case cannot settle; bullet 2 retitled "*Tension-dissolution* as candidate legitimacy move" with closing sentence offering the move for examination inside Suchman's typology rather than asserting it as a category; bullet 3 (Sovereignty Seal) closing sentence reframed as "a question for follow-on work" rather than asserting an extension to the certification literature; bullet 4 retitled "*Circular market validation* as candidate structural property" with closing sentence noting standing as a structural property of the class requires close readings of further cases. Fifth observation (Pattern F triple-load) preserved (already correctly hedged); "The finding" → "The observation". New closing paragraph added at the end of the section stating what would be needed to lift the five from analytical observation to theoretical contribution (broader empirical base + closer engagement with how the existing constructs already handle the phenomena), pointing to `\Cref{sec:conc-future}`. |
| `06_report/Chapters/06_conclusion.tex` | Edit (§`sec:conc-contrib` Theoretical paragraph) | "Four contributions to the institutional-logics literature" → "Four analytical observations from the corpus carry theoretical interest and are offered for testing on broader empirical material rather than as settled contributions to the literature". Per-item claim verbs softened in parallel with Ch 5: "is generally useful" → "may be useful"; "merits naming inside Suchman's cognitive-legitimacy typology as distinct" → "is offered for examination inside Suchman's cognitive-legitimacy typology as a candidate move distinct"; "is advanced as a portable field-bridging legitimacy artefact" → "is described as a portable field-bridging legitimacy artefact"; "is offered as a structural property" → "is offered as a candidate structural property". Closing sentence added pointing to `\Cref{sec:conc-future}` for the cross-case work that would test whether any of the five hold beyond the single Work Programme. Empirical and Practical paragraphs unchanged. |

### Files unchanged

`06_report/main.tex`, `bibliography.bib`, `voice_card.md`, Chs 2–4, Setup/Frontmatter/Backmatter other than the Abstract. No labels broken; brace counts preserved on every edited file (Abstract 6/6, Ch 1 63/63, Ch 5 81/81, Ch 6 45/45). All four `\Cref{sec:conc-future}` references introduced by this batch resolve to the existing label in `06_conclusion.tex`.
...
## Batch 58 — Full sweep of orientation docs: declare report draft complete and in review

**Date:** 2026-06-29
**Approved by:** user message "fix all of them, and consider the report draft done and in review", following a validation pass that surfaced eight stale or incomplete orientation files (six beyond what Batch 52 had closed).
**Type:** Content edit (eight files; status / folder-map / checklist refresh — no chapter prose, codebook, synthesis, or bibliography touched).

### Rationale

Batch 52 (2026-06-28) refreshed `README.md`, root `CONTEXT.md`, and `06_report/CONTEXT.md` to reflect Pass 3 underway, but described the report as "substantially drafted and under active revision" and re-affirmed several pre-submission items (sXXXXXX, co-supervisor, address verification) as still open. Since then, Batches 56 and 57 filled `Setup/Statics.tex` with the student number `s243778`, set `\thesissupervisor{Maria-Theresa Norn}`, corrected the supervisor affiliation to DTU Entrepreneurship, and dropped the `\thesiscosupervisor` macro entirely — closing those checklist items. A second sweep against the repository state today also surfaced gaps Batch 52 missed: the root `CLAUDE.md` folder map omitted `01_literature/cited/`, `01_literature/output/`, and several `06_report/` artefacts (`voice_card.md`, `methodology_revision_spec.md`, `dtu-template/`, `figures/`, `sections/`, `readme.md`); the per-stage CONTEXT files in `00_admin/`, `01_literature/`, and `03_corpus/` still carried the original "placeholder — to be drafted" status from Batch 00 scaffolding even though all three folders are populated; and the file maps inside `06_report/CLAUDE.md` and `06_report/CONTEXT.md` did not list `voice_card.md` (despite being referenced as the authoritative prose voice elsewhere in the same files) or any of the other present-but-unlisted artefacts. The user has now also declared the report draft complete and in review, which updates the status language across the orientation layer.

### Changes by file

| File | Action | Note |
|---|---|---|
| `CLAUDE.md` (root) | Edit (×4) | (i) Folder map: `01_literature/` now lists `cited/` (with own README) and `output/` alongside `references/` and notes the cited-vs-references split. (ii) Folder map: `06_report/` now lists `voice_card.md`, `methodology_revision_spec.md`, `readme.md`, `dtu-template/main.pdf`, `figures/`, `sections/`; header annotation changes from "DTU LaTeX template — compile main.tex" to "DTU LaTeX template — compile main.tex; draft complete, in review". (iii) Triggers row for `status` rewritten from "list which of Chs 1, 2, 4, 5, 6 are still to draft" to "report draft complete (Chs 1–6 + appendix); list open review items from `06_report/CONTEXT.md` §9". (iv) Stage Handoffs Pass 3 annotation changed from "current; Ch 3 methodology drafted; Chs 1, 2, 4–6 to draft" to "draft complete; in review (all six chapters + appendix drafted)". |
| `CONTEXT.md` (root) | Edit (×2) | (i) "Current stage" line: Pass 3 changed from "current" to "draft complete, in review". (ii) Stage map rewritten across rows for `00_admin/`, `01_literature/`, `03_corpus/`, `04_codebook/`, and `06_report/` to reflect actual contents and the new draft-complete-in-review status for stage 06. Stage 02 and 05 rows unchanged. |
| `00_admin/CONTEXT.md` | Edit | Replaced "placeholder — to be drafted" status with a description of the present files (`Project_Plan__Thesis.pdf` v0 proposal, `Learning objectives.docx`) and named the DTU submission-extras still to add (revised plan, auto-evaluation). "What lives where" expanded to list the two actual files; `output/`, `_scratch/`, `references/` retained as "(if created)" slots. |
| `01_literature/CONTEXT.md` | Edit | Replaced "placeholder — to be drafted" status with current state (background `references/` collected; `cited/` mirrors bib with 15 of 19 PDFs; 4 missing per `cited/README.md`; reading and bib expansion ongoing). "What lives where" extended with new entries for `cited/`, `output/`, and the stray `references/ec2024eicwp.pdf` left after the cite-key rename to `ec2025eicwp`. |
| `03_corpus/CONTEXT.md` | Edit | Replaced "placeholder — to be drafted" status with "Complete" + no-delete/no-rename caveat. "What lives where" expanded from two generic bullets to a four-bullet listing of the actual files: full WP 2026, sliced Path+STEP subset, Horizon Europe Regulation, plain-text extraction. |
| `06_report/CLAUDE.md` | Edit (×3) | (i) File map: two new rows for `voice_card.md` and `methodology_revision_spec.md` inserted after `main.tex`. (ii) File map: three new rows for `dtu-template/main.pdf`, `figures/`, `sections/` inserted before the existing `readme.md` row. (iii) §Final-build checklist #2 rewritten — dropped the stale "(currently `sXXXXXX`)" and `\thesiscosupervisor` references, replaced with a confirm-not-fill action listing the actual filled values from `Setup/Statics.tex` and noting the cosupervisor macro was dropped in Batch 56. |
| `06_report/CONTEXT.md` | Edit (×4) | (i) §3 Structure tree: `voice_card.md` and `methodology_revision_spec.md` added at the top of the tree. (ii) §3 Structure tree: `dtu-template/main.pdf`, `figures/`, `sections/` added after `Pictures/`. (iii) §7 Final-build checklist #2: same rewrite as `06_report/CLAUDE.md` #2. (iv) §9 Status section rewritten end-to-end: Pass 3 marked "draft complete; in review"; per-chapter line annotations cross-reference the methodology revision spec and Batch 51's contributions cut; "Outstanding pre-submission items" split into "Closed" (Statics values from Batches 56–57; cosupervisor drop from Batch 56) and "Open review items" (subtitle, bib expansion, DTU submission-extras, final-build cleanup). |
| `README.md` | Edit | Status section's last sentence changed from "Pass 3 (thesis writing) underway — all six chapters and the appendix are substantially drafted in `06_report/` and under active revision" to "Pass 3 (thesis writing) — **draft complete; in review.** All six chapters and the appendix are drafted in `06_report/`; open review items are tracked in `06_report/CONTEXT.md` §9". |

### Files unchanged

`02_theory/CONTEXT.md`, `04_codebook/CONTEXT.md`, `05_analysis/CONTEXT.md` — re-read and confirmed already accurate. `06_report/voice_card.md`, `06_report/methodology_revision_spec.md`, `01_literature/cited/README.md` — content authored elsewhere, no orientation-doc edits needed. No chapter prose, codebook content, synthesis content, or bibliography touched in this batch.

### Validation

The eight edited files are markdown orientation documents; no compile to verify. Cross-checked the status claims against (a) `ls` listings of `01_literature/`, `03_corpus/`, `06_report/`, and `00_admin/` confirming the file inventories now described, (b) `Setup/Statics.tex` confirming `\studentnumber{s243778}`, `\thesissupervisor{Maria-Theresa Norn}`, `\department{DTU Entrepreneurship}`, `\addressI{Diplomvej, Building 372A}`, `\addressII{2800 Kgs. Lyngby}`, `\departmentwebsite{entrepreneurship.dtu.dk}`, and the absence of any `\thesiscosupervisor` macro, (c) `git log --oneline -8` confirming Batch 56 dropped the cosupervisor block and Batch 57 corrected the supervisor affiliation, and (d) `01_literature/cited/README.md`'s own summary line (15 PDFs present, 4 missing) used as the authoritative count.
