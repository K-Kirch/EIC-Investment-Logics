# Defence Brief

## When and where

- **Date:** Friday 10 July 2026
- **Time:** 15:30
- **Venue:** [TBD — confirm room / online link with DTU Entrepreneurship]
- **Duration budget:** ≤ 30 min presentation + 30 min examination (Q&A + deliberation)

## Who

- **Candidate:** Kristian Bonde Kirch (s243778)
- **Supervisor / internal examiner:** Maria-Theresa Norn (DTU Entrepreneurship)
- **External censor:** [TBD]

## Format

DTU MSc Technology Entrepreneurship kandidatspeciale defence:
- Candidate presents (target ~22:30 per `outline.md` — supervisor's guidance is that most candidates don't fill the 30-min slot).
- Panel questioning (~30 min). Both supervisor/internal examiner and external censor have read the submitted thesis.
- Panel deliberates in private; grade issued on the DTU 7-point scale.

## Assessed against

DTU MSc Technology Entrepreneurship learning objectives (see `../00_admin/Learning objectives.docx`). The defence tests the candidate's ability to:
- Delineate the topic and justify scope.
- Justify theory selection.
- Substantiate methods.
- Discuss results critically.
- Extract implications.
- Reflect on limitations.

## Presentation time budget

Source of truth: `outline.md` v2 (7-beat crescendo, ~22:30 target, ~7:30 slack in the 30-min slot). Audience-has-read assumption drives the compression — Beats 2 and 3 skip exposition. Do not restore the 8-block breakdown here; if outline.md changes, update it there.

## Open items to confirm before Friday

- [ ] Room / online link
- [ ] External censor identity
- [ ] Slide format decision (Beamer vs PPTX)
- [ ] Whether a printed handout is expected
- [ ] Test AV setup in room if in-person
