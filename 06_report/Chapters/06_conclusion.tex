...
The sub-questions framed the inquiry as a textual one and are restated
verbatim from \Cref{sec:intro-rq}:
...
The empirical scope was documentary throughout: the Pathfinder and STEP
Scale Up sections of the Work Programme, read against the regulatory
instruments that establish them, with no interview material.
...
logic at TRL~1--4; STEP Scale Up is organised primarily by a state
logic operating through market mechanics at the scale-up end of the
pipeline. The macro-level succession Science\,$\to$\,Market\,$\to$
\,State is reproduced inside every Challenge chapter as Pattern~D, so
that substantively different technology domains receive the same
four-step logic order. The reproduction is a feature of the chapter
template rather than a derivation from the science.
...
taken-for-grantedness of bureaucratic prose itself; its most
consequential instance is [082], where state-defined Challenge
relevance is embedded as the primary sub-criterion of scientific
excellence. Pattern~F shows the Technology Readiness Level framework
carrying three legitimation axes within a single designed artefact,
so that the political question of where public money should flow in
the pipeline is presented as a technical question of developmental
readiness. The Sovereignty Seal operates as a designed credential
that carries state-issued legitimacy across institutional fields
independently of the capital decision that produced it. The three
strategies operate at different scales (sentence, artefact,
credential) and together perform the bulk of the text's legitimation
work.
...
\textcite{mazzucato2013entrepreneurial}'s patient-capital account; it is
...
mechanisms rather than through a universal insert. The model visible in
the 2026 Work Programme is one in which mission-oriented investment in
Pathfinder and STEP is carried by an architecture that templates
cross-cutting normative priorities, mediates between political
principal and applicant through hired technocrats with contractual
discretion, and combines borrowed market vocabulary with state-anchored
structural design.

\section{Contributions revisited}
\label{sec:conc-contrib}

The contributions stated in \Cref{sec:intro-contribution} can now be
restated against the evidence that supports them.
...
\paragraph{Theoretical and methodological.}
A subsidiary contribution concerns how the three isomorphism mechanisms
are identified in a documentary corpus. Coercive and normative
pressures leave named traces in the Work Programme (regulations cited,
evaluation communities invoked, standards named); mimetic pressures do
not, and their identification depended on matching the instrument's
structural features against comparators supplied by the
developmental-state literature. The asymmetry is a property of
\textcite{dimaggio1983iron}'s mechanisms rather than an artefact of
this corpus, and it has consequences for how mimetic isomorphism can be
argued from a text at all. The thesis records the identification
strategy explicitly (\cref{sec:meth-pass1}) and reads the mimetic
findings against a named comparator set (\cref{sec:bg-it-isomorphism})
rather than leaving them implicit. The methodological point is portable
to other close readings of policy instruments in which the imitated
model is not named in the text.
...
Guidance Funds, and the Mazzucato patient-capital model) are supplied
by the developmental-state literature to characterise
mission-oriented design. The Work Programme names none of them; its
benchmarking vocabulary is generic (Strategic Goal 4: matching and
surpassing the performance of the USA and Asia). The comparators are
used in this thesis to read the EIC's instrument design against a
recognised comparator set rather than subjected to the same close
reading as the EIC text, and the mimetic-isomorphism reading at
Pattern~F and at \Cref{sec:disc-step-mimesis} relies on the secondary
literature for the characterisation of the referent fields and on
structural resemblance for their identification in the corpus.
