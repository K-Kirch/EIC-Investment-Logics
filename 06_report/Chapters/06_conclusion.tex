...
Four analytical observations from the corpus carry theoretical
interest and are offered for testing on broader empirical material
rather than as settled contributions to the literature. The
\emph{logic-as-register} distinction (Pass~2 coding rule~7) separates
the vocabulary an instrument speaks from the logic that organises
it, and may be useful wherever the state borrows market discourse
without transferring authority to market actors. \emph{Tension-dissolution}
(Pattern~A) names the textual move that presents conflicting logics
as naturally complementary, and is offered for examination inside
Suchman's cognitive-legitimacy typology as a candidate move distinct
from surface-visible logic conflict and from
\textcite{friedland1991bringing}'s mobilisation of contradiction. The
Sovereignty Seal is described as a portable field-bridging legitimacy
artefact: a designed credential whose legitimacy is engineered to
travel across institutional fields, decoupled from the capital
decision that produced it. \emph{Circular market validation}
(Pattern~E) names the configuration of simultaneous market-interest
and market-failure requirements satisfiable only through state-anchor
participation, and is offered as a candidate structural property of
the class of developmental-state instruments to which STEP belongs.
A fifth, more modest observation is that Pattern~F's triple-load
shows \textcite{scott2014institutions}'s three pillars co-operating
within a single designed artefact, refining rather than contradicting
Scott's frame. \Cref{sec:conc-future} sets out the cross-case work
that would test whether any of the five hold beyond the single Work
Programme on which they are grounded.
