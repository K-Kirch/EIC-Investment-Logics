...
\parencite{ec2024eicwp}. The EIC's mandate is breakthrough
...
returned to in \cref{sec:results-step} and \cref{sec:discussion-seal}.
...
STEP and the broader EU instrument vocabulary explicitly benchmark
against external comparators: the United States DARPA model is named
in the EIC's own self-description; Singaporean Temasek-style
sovereign investment vehicles are invoked in the strategic-autonomy
debate that produced STEP; and the wider Government Guidance Fund
literature on Chinese practice (\cref{sec:bg-mazzucato}) sits in the
background of the European catalytic-anchor design. These comparators
are returned to in \cref{sec:discussion-mimesis}.
...
of departure is \textcite{mazzucato2018mission}, which argues that the
...
literature on Chinese GGFs and on Singapore's Temasek model develops
this design at length \todoinline{add bib entries: ban2019 on
developmental financial statecraft; CSET report on Chinese GGFs; a
Temasek source}. The point for the present chapter is not to
adjudicate the European case against the Asian comparators, but to
flag that STEP's anchor-investor design, equity-only mechanics, and
strategic-technology scope are recognisable in this comparative
literature and will be read against it in \cref{sec:discussion-developmental}.
...
\cref{sec:discussion-stepmechanics} to read STEP's circular market
...
coercive; DARPA, Temasek and the Government Guidance Fund as mimetic
references; the freedom-to-operate analysis and the business-plan
format as normative. Scott's three pillars are used selectively to
...
\begin{figure}[H]
    ...
    \fbox{\parbox[c][6cm][c]{0.85\textwidth}{\centering
        ...
        Diagram of the analytical framework linking the four
        institutional logics, the three isomorphic mechanisms, the
        three legitimacy types and Scott's three pillars to the EIC
        instrument architecture (eligibility, evaluation, governance,
        impact) and to the text-level signatures used in coding.}}
    \caption[Analytical framework]{Analytical framework guiding the
    empirical analysis.}
    \label{fig:analytical-framework}
\end{figure}
\todoinline{Replace the placeholder with a real diagram. Suggested
layout: three concentric bands (logics inner; mechanisms and pillars
middle; legitimacy types outer) intersected by two columns
representing Pathfinder and STEP, with the Sovereignty Seal as a
bridging arrow between the columns.}
