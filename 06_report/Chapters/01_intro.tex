...
\todoinline{anchor in Draghi 2024; Letta 2024; JRC --- add to bib}. In
...
\begin{itemize}
    \item \textbf{Theoretical contribution.} The thesis extends
          institutional-logics scholarship into supranational
          public-investment instruments, a setting the tradition has
          largely passed over, and articulates two analytical
          refinements that emerge from the corpus reading. The first,
          \emph{logic-as-register}, distinguishes the vocabulary an
          instrument speaks from the logic that organises it: a passage
          can be saturated with market terminology while remaining
          structured by a state logic. The second,
          \emph{tension-dissolution}, names the textual move by which
          logics in potential conflict are presented as complementary,
          so the conflict never has to be visibly arbitrated. Two
          further institutional devices identified in the corpus, the
          Sovereignty Seal as a portable, field-bridging legitimacy
          artefact and \emph{circular market validation} as a
          developmental-state mechanism in which the state constructs
          the market signal it then requires, are advanced as candidate
          contributions to the wider literature on institutional
          devices.
