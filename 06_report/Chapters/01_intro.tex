...
\begin{itemize}
    \item \textbf{Theoretical contribution.} The thesis applies
          institutional-logics scholarship to supranational
          public-investment instruments, a setting the tradition has
          largely passed over. From the corpus reading it develops
          two analytical distinctions that warrant testing on broader
          empirical material. The first, \emph{logic-as-register},
          distinguishes the vocabulary an instrument speaks from the
          logic that organises it: a passage can be saturated with
          market terminology while remaining structured by a state
          logic. The second, \emph{tension-dissolution}, names the
          textual move by which logics in potential conflict are
          presented as complementary, so the conflict never has to be
          visibly arbitrated. Two further institutional devices
          identified in the corpus, the Sovereignty Seal as a
          portable, field-bridging legitimacy artefact and
          \emph{circular market validation} as a developmental-state
          mechanism in which the state constructs the market signal
          it then requires, are advanced as observations from the
          corpus rather than as settled contributions to the
          literature on institutional devices. All four are grounded
          in a single Work Programme; the cross-case work that would
          lift them to settled contributions is flagged at
