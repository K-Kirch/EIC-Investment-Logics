...
Government Guidance Fund model than to the DARPA model. The GGF
structure is built around state-anchor participation with private
co-investment, operating through state-controlled administrative
architecture and oriented toward state-defined strategic priorities.
That description fits STEP more cleanly than any DARPA-like reading
does. The DARPA comparator nonetheless remains analytically useful:
it names the mimetic-borrowing dynamic in vocabulary EU policy
discourse recognises and can debate, whereas the GGF comparator sits
in a register EU policy discourse rarely engages directly. The
comparator set is therefore doing asymmetric analytical work --- the
GGF supplying the structural resemblance, the DARPA reference
supplying the recognised institutional vocabulary.

The cross-link to Pattern~F (\cref{sec:disc-trl}) is that the mimetic
reading is not confined to STEP. The TRL ladder gates Pathfinder and
carries US-defence provenance in its NASA and DoD adoption history.
The EIC's institutional logic is therefore partly legible as mimetic
borrowing from US innovation finance and US-defence procurement,
operating at two different points in the pipeline. The borrowing is
selective in both cases, and the corpus names neither comparator;
the reference fields supply legitimacy and design elements that the
EU procurement architecture then absorbs.
