...
corpus as a whole, sets out four analytical observations the corpus
supports, draws practical implications for three audiences with a
stake in the instruments, and closes with the scope conditions under
which the findings travel.
...
The corpus supports four observations of interest to the
institutional-theoretic literature on supranational public-investment
instruments. Each is grounded in a single Work Programme and is
offered as an observation to be tested on broader empirical material,
not as a settled theoretical contribution.

\begin{enumerate}[leftmargin=*]
  \item \textbf{\emph{Logic-as-register} as analytical distinction.}
    The \emph{logic-as-register} distinction (rule~7 in the Pass~2
    codebook) treats market vocabulary as analytically distinct from
    market organising logic. It earns its keep in the EIC corpus,
    where state-organised passages are saturated with market terms
    without authority transferring to market actors. Whether the
    distinction holds across other supranational public-investment
    instruments is an empirical question a single case cannot settle.
  \item \textbf{\emph{Tension-dissolution} as candidate legitimacy
    move.} Pattern~A names the textual move that presents conflicting
    logics as naturally complementary, observed across the corpus as
    the inverse of surface-visible logic conflict (the
    \texttt{LOGIC\_TENSION} code) and as a different move from
    \textcite{friedland1991bringing}'s mobilisation of contradiction.
    The move operates pre-discursively through bureaucratic prose
    conventions. Whether it warrants standing as a separate category
    inside Suchman's cognitive-legitimacy typology requires more cases
    than the present corpus provides; the observation is offered for
    that examination.
  \item \textbf{The Sovereignty Seal as a portable field-bridging
    ...
    decoupled at [166] from the capital decision that produced it.
    The artefact sits outside the side-effect-of-regulation accounts
    that dominate the literature on state certification. Whether the
    Seal is a category extension or a single instance worth tracking
    across subsequent EU innovation instruments is a question for
    follow-on work.
  \item \textbf{\emph{Circular market validation} as candidate
    structural property of developmental-state investment design.}
    Pattern~E reads as a recognisable feature of the class of
    instruments to which STEP belongs rather than as a flaw of STEP,
    consistent with \textcite{mazzucato2018mission}'s patient-capital
    account and with secondary-literature descriptions of Chinese
    Government Guidance Funds. The label \emph{circular market
    validation} names the configuration of simultaneous
    market-interest and market-failure requirements satisfiable only
    through state-anchor participation. The observation is grounded
    in a single instrument; its standing as a structural property of
    the class requires close readings of further cases.
\end{enumerate}
...
they can co-operate within a single designed object. The observation
refines rather than contradicts Scott's frame.

Lifting any of these five from analytical observation to theoretical
contribution requires two moves the present design cannot make. The
first is a broader empirical base: comparative readings of further EU
instruments, or of non-EU mission-oriented agencies using the same
coding architecture, would test whether the moves described here
recur or are EIC-specific. The second is closer engagement with how
the existing constructs (Thornton's interinstitutional system on
register, Suchman's typology of cognitive legitimacy, the
certification literature on portable credentials) already handle the
phenomena named here, so that what the corpus adds is stated more
sharply. \Cref{sec:conc-future} sets out the most immediate routes
for both moves.
